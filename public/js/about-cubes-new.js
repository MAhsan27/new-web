(function () {
  ...
  function initScene() {
    ...
    var material = new THREE.ShaderMaterial({
      uniforms: {
        uBaseColor: { value: new THREE.Vector3(0.043, 0.059, 0.169) }
      },
      vertexShader: [
      ...
      fragmentShader: [
        ...
        '  vec3 blackBase = uBaseColor * (0.88 + 0.22 * diff);',
    ...
    function blendPalette(t) {
      ...
      if (t < 0.35) {
        ...
        col.lerpColors(blueDeep, blue, (t - 0.35) / 0.35);
      } else {
    ...
    // var blueDeep   = new THREE.Color(0x2540c9);
    // var blue       = new THREE.Color(0x4d6cfa);
    // var blueLite   = new THREE.Color(0x7aa2ff);
    // var orange     = new THREE.Color(0xffab1e);
    // function blendPalette(t) {
    //   var col = new THREE.Color();
    //   if (t < 0.35) { col.lerpColors(blueDeep, blue, t / 0.35); }
    //   else if (t < 0.70) { col.lerpColors(blue, blueLite, (t - 0.35) / 0.35); }
    //   else { col.lerpColors(blueLite, orange, (t - 0.70) / 0.30); }
    //   return col;
    // }
    ...
    // Keep the cube base color in sync with the light/dark toggle.
    // Dark = deep navy (original look). Light = soft off-white so the
    // grid stays visible without turning into a black patch on white.
    var darkBaseColor  = new THREE.Vector3(0.043, 0.059, 0.169);
    var lightBaseColor = new THREE.Vector3(0.93, 0.945, 0.97);
    function applyThemeColor() {
      var isLight = document.body.getAttribute('data-theme') === 'light';
      material.uniforms.uBaseColor.value.copy(isLight ? lightBaseColor : darkBaseColor);
    }
    applyThemeColor();
